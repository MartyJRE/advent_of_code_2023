fun main() {
    ...
    fun part1(input: List<String>): Int {
        val data = input
            .map { it.replace(Regex("""Card +\d+:"""), "").trim() }
            .map { line -> line.split("|").map { side -> side.trim() } }
            .map {
                arrayListOf(
                    it[0].split(Regex(""" +""")).map { num -> num.trim().toIntOrNull() ?: 0 },
                    it[1].split(Regex(""" +""")).map { num -> num.trim().toIntOrNull() ?: 0 }
                )
            }
        var total = 0
        for (card in data) {
    ...
    fun part2(input: List<String>): Int {
        ...
        for (line in input) {
            for (matchResult in Regex("""Card +(\d+): +(.*) \| (.*)""").findAll(line)) {
                val id = matchResult.groupValues[1].toInt()
                val winning = matchResult.groupValues[2].trim().split(Regex(""" +""")).map { it.trim().toInt() }
                val guesses = matchResult.groupValues[3].trim().split(Regex(""" +""")).map { it.trim().toInt() }
                var matching = 0
